...
(function (cloudPOS) {
    var defineHeaders = function ($httpProvider, $translateProvider, ResourceFactoryProvider, HttpServiceProvider, $idleProvider, $keepaliveProvider, IDLE_DURATION, WARN_DURATION, KEEPALIVE_INTERVAL) {
        ...
        var baseApiUrl = "http://localhost/test/";
        var host = "";
        var portNumber = "";
        //accessing from server

        if (mainLink.hostname.indexOf('intahub.com') >= 0) {
            ...
            if (domains[0] == "demo") {
                //$httpProvider.defaults.headers.common['Platform-TenantId'] = 'default';
                ResourceFactoryProvider.setTenantIdenetifier('default');
                console.log("demo server", domains[0]);
            } else {
                //$httpProvider.defaults.headers.common['Platform-TenantId'] = domains[0];
    ...
    cloudPOS.ng.application.config(defineHeaders).run(function ($log, $idle) {
